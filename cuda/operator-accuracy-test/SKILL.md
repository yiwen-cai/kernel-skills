...
### 2. 写算子（src/<op>/ 下 .cu + kernel_<op>.cuh）

每算子在 `src/<op>/` 下拆两个文件，**禁止复制粘贴 kernel**：
...
要点：自包含、`#pragma once`、namespace 包裹、函数内 `using namespace`、模板跨 TU 安全/非模板用 `inline`。测试文件（在 `tests/`）用 `#include "<op>/kernel_<op>.cuh"` 调真 kernel（绝对路径风格，靠 `-I src` 解析），源改了测试自动同步。完整范例见 [assets/kernel_vecAdd.example.cuh](assets/kernel_vecAdd.example.cuh)。

### 3. 生成测试骨架

把 [assets/new_accuracy_test.sh](assets/new_accuracy_test.sh) 拷到项目 `scripts/`，`./scripts/new_accuracy_test.sh <op>` 生成 `tests/test_<op>.cu`。骨架已实现共用部分：
...
填 `<op>_cpu`（手写 host fp32 reference）、三类输入（均匀/正态/边界，固定 `srand(42)`）、各 case 的 `run_case`（alloc→launch→D2H→reference→compare→print）。编译运行 `./scripts/cc.sh tests/test_<op>.cu`，全 PASS 且退出码 0。
